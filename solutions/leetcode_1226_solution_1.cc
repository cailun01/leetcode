...
示例：
输入：n = 1
输出：[[4,2,1],[4,1,1],[0,1,1],[2,2,1],
       [2,1,1],[2,0,3],[2,1,2],[2,2,2],
       [4,0,3],[4,1,2],[0,2,1],[4,2,2],
       [3,2,1],[3,1,1],[0,0,3],[0,1,2],
       [0,2,2],[1,2,1],[1,1,1],[3,0,3],
...
public:
    ...
    void wantsToEat(int philosopher,
        ...
        int left_fork = philosopher; //左边的叉子 的编号
        int right_fork = (philosopher + 1) % 5; //右边的叉子 的编号
        guid.lock();        //进入临界区
        lock[left_fork].lock(); //拿起左边的叉子
        lock[right_fork].lock(); //拿起右边的叉子
        pickLeftFork(); //拿起左边的叉子 的具体执行
        pickRightFork(); //拿起右边的叉子 的具体执行
        guid.unlock(); //退出临界区
        eat(); //吃意大利面 的具体执行
        putRightFork(); //放下左边的叉子 的具体执行
        putLeftFork(); //放下右边的叉子 的具体执行
        lock[left_fork].unlock(); //放下左边的叉子
        lock[right_fork].unlock();  //放下右边的叉子
    }
private:
    std::mutex lock[5];
    std::mutex guid;
