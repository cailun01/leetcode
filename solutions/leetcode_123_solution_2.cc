...
public:
  int maxProfit(vector<int>& prices) {
    ...
    // profitOne0表示在第i天结束时，最多进行1次交易且在进行操作后持有0份股票的情况下可以获得的最大收益；
    // profitOne1表示在第i天结束时，最多进行1次交易且在进行操作后持有1份股票的情况下可以获得的最大收益。
    // profitTwo0表示在第i天结束时，最多进行2次交易且在进行操作后持有0份股票的情况下可以获得的最大收益；
    // profitTwo1表示在第i天结束时，最多进行2次交易且在进行操作后持有1份股票的情况下可以获得的最大收益。
